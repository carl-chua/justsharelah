    ...
    try {
        await firebase.firestore().runTransaction(async (tn) => {
            ...
            if(user1Doc.get("following") != null) {
                console.log("LINE1")
                tn.update(user1Ref, {following : firebase.firestore.FieldValue.arrayUnion(user2Id)})
            } else {
                console.log("LINE2")
            ...
            if(user2Doc.get("followers") != null) {
                console.log("LINE3")
                tn.update(user2Ref, {followers : firebase.firestore.FieldValue.arrayUnion(user1Id)})
...
export async function unfollowUser(user1Id, user2Id) {
    ...
    try {
        await firebase.firestore().runTransaction(async (tn) => {
            ...
            if(user1Doc.get("following") != null) {
                console.log("LINE1")
                tn.update(user1Ref, {following : firebase.firestore.FieldValue.arrayRemove(user2Id)})
            }

            if(user2Doc.get("followers") != null) {
                console.log("LINE3")
                tn.update(user2Ref, {followers : firebase.firestore.FieldValue.arrayRemove(user1Id)})
